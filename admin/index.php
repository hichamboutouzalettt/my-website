<?php
	session_start();
	require_once 'db_action/csrf_token.php';
						...
						<div class="dashboard-content-container cwide">								
							...
							<div class="dashboard-content-panel-wrapper dfgw-mt">
								<div class="dashboard-content-panel-header">
									...
									<p>Overview of existing apps in your list. You can re-arrange apps by dragging them or delete a single app by clicking on "Delete" button in app row.</p>
								</div>
								<div class="dashboard-content-panel-content">
									<input type="hidden" id="sort_csrf_token" value="<?php echo generateCSRFToken(); ?>">
										...
										?>									
											<div id='sort_<?php echo $data['id'] ?>' class="dashboard-app-row-item">
												...
												<div class="dashboard-app-item-info-wrapper">
													...
													<h4 class="app_name"><?php echo htmlspecialchars($data['app_name']); ?></h4>
													<p><?php echo htmlspecialchars($data['app_description']); ?></p>
													<?php if (htmlspecialchars(isset($data['app_locker_url']))) {?><div class="dashboard-app-item-locker-url"> <?php echo "Locker URL: <strong>" . htmlspecialchars($data['app_locker_url']); ?></strong></div><?php } ?>
												</div>
						...
						<div class="dashboard-content-container">								
							...
							<div class="dashboard-content-panel-wrapper dfgw-mt">
								<div class="dashboard-content-panel-header">
									...
									<p>Enter your Google Analytics tracking ID to include the tracking code. This supports both Universal Analytics (UA-XXXXXXXX-X) and Google Analytics 4 (G-XXXXXXXXXX) property IDs. Leave the field empty to exclude the tracking code.</p>
								</div>
								<form action="db_action/u_g_s.php" method="POST">
									<input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>">
									<div class="dashboard-content-panel-content">
										<div class="form-group">
											...
											<div class="mr-input-wrapper">
												...
												<input type="text" name="ga_id" class="mr-input-style mr-input-style-icon" placeholder="G-XXXXXXXXXX or UA-XXXXXXXX-X" value="<?php echo isset($data2['ga_id']) ? htmlspecialchars($data2['ga_id']) : ''; ?>" />
											</div>                                                                
										</div>  
										<?php if(isset($data2['ga_id']) && $data2['ga_id'] !== '') : ?>  
										<div class="input-instructions"><i class="material-icons-two-tone">info</i><div>The following code snippet will be automatically included in your main index.php file just before the closing <code>&lt;/head></code> tag:</div></div>
										<div class="code-e-wrapper">
											<code>
												<!-- Global site tag (gtag.js) - Google Analytics -->
												&lt;script async src="https://www.googletagmanager.com/gtag/js?id=<?php echo htmlspecialchars($data2['ga_id']); ?>">&lt;/script>
												&lt;script>
												  window.dataLayer = window.dataLayer || [];
												  function gtag(){dataLayer.push(arguments);}
												  gtag('js', new Date());             
												  gtag('config', '<?php echo htmlspecialchars($data2['ga_id']); ?>');
												&lt;/script>
											</code>
										</div>
										<?php endif; ?>
							...
							<div class="dashboard-content-panel-wrapper">
								...
								<form id="n-p-form" action="db_action/uld.php" method="POST">
									...
									<div class="dashboard-content-panel-content">
										...
										<div class="mr-field-wrapper">
											<label for="new_password_confirm" class="mr-dashboard-label">Repeat new admin password</label>
											<div class="mr-input-wrapper">
												<span class="material-icons-two-tone">vpn_key</span>
